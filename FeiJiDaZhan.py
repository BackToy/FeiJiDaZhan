...
class BasePlane(Base):
    """飞机基类"""
    def __init__(self, plane_type, screen_temp, x, y, image_name, picture_num,
                 HP_temp):
    ...
    def display(self):
        ...
        if self.hitted == True and self.image_index < self.bomb_picture_num and self.HP <= 0:
            ...
            if self.picture_count == plane_bomb_time[
                    self.plane_type]:  # 根据飞机类型不同，爆炸效果持续的时间不同
    ...
    def crate_images(self, bomb_picture_name):
        for i in range(1, self.bomb_picture_num + 1):
            self.bomb_picture_list.append(
                pygame.image.load("./img/" + bomb_picture_name + str(i) +
                                  ".png"))
    ...
    def fire(self, bullet_maximun):
        if self.HP > 0:
            ...
            if (random_num == 10 or random_num
                    == 45) and len(self.bullet_list) < bullet_maximun:
                self.bullet_list.append(
                    EnemyBullet(self.screen, self.x, self.y, self))
...
class HeroPlane(BasePlane):
    ...
    def __init__(self, screen_temp):
        BasePlane.__init__(self, 3, screen_temp, 210, 728, "./img/hero1.png",
                           4, HP_list[3])  # super().__init__()
            ...
            if len(self.key_down_list) == 2:  # 两个键
                if (
                    ...
                    self.move_left_and_up()
                      ...
                      == K_UP) or (self.key_down_list[1] == K_RIGHT
                                   and self.key_down_list[0] == K_UP):
                    self.move_right_and_up()
                elif (self.key_down_list[0] == K_LEFT and self.key_down_list[1]
                      == K_DOWN) or (self.key_down_list[1] == K_LEFT
                                     and self.key_down_list[0] == K_DOWN):
                    self.move_lef_and_down()
                elif (self.key_down_list[0] == K_RIGHT
                      and self.key_down_list[1]
                      == K_DOWN) or (self.key_down_list[1] == K_RIGHT
                                     and self.key_down_list[0] == K_DOWN):
    ...
    def press_fire(self):
        if len(self.bullet_list) == 0 and len(self.space_key_list):
            ...
            if len(self.space_key_list) != 0:
                if self.bullet_list[len(self.bullet_list) -
                                    1].y < self.y - 14 - 60:
                    self.fire()
    ...
    def fire(self):
        ...
        if not self.is_three_bullet:
            if len(self.bullet_list) < plane_maximum_bullet[
                    self.plane_type]:  # 单发炮台子弹限制为8
                self.bullet_list.append(
                    Bullet(self.screen, self.x + 40, self.y - 14, self))
        else:  # 没有子弹限制
            # 主炮管
            self.bullet_list.append(
                Bullet(self.screen, self.x + 40, self.y - 14, self))
            # 创建２，３号炮管子弹
            self.barrel_2.append(
                Bullet(self.screen, self.x + 5, self.y + 20, self))
            self.barrel_3.append(
                Bullet(self.screen, self.x + 75, self.y + 20, self))
    ...
    def supply_hitted(self, supply_temp, width, height):  #  widht和height表示范围
        if supply_temp and self.HP:
            ...
            supply_temp_left_x = supply_temp.x + supply_size[
                supply_temp.supply_type]["width"] * 0.15
            supply_temp_right_x = supply_temp.x + supply_size[
                supply_temp.supply_type]["width"] * 0.85
            supply_temp_top_y = supply_temp.y + supply_size[
...
class Enemy0Plane(BasePlane):
    ...
    def __init__(self, screen_temp):
        ...
        BasePlane.__init__(self, 0, screen_temp, random_num_x, random_num_y,
                           "./img/enemy0.png", 4, HP_list[0])
        BasePlane.crate_images(self,
                               "enemy0_down")  # 第二个参数为飞机的plane_type, 0为enemy0
...
class Enemy1Plane(BasePlane):
    """enemy1的类"""
    def __init__(self, screen_temp):
        BasePlane.__init__(self, 1, screen_temp, 205, -90, "./img/enemy1.png",
                           4, HP_list[1])
...
class Enemy2Plane(BasePlane):
    """enemy2的类"""
    def __init__(self, screen_temp):
        BasePlane.__init__(self, 2, screen_temp, 158, -246, "./img/enemy2.png",
                           5, HP_list[2])
...
class Bullet(BaseBullet):
    ...
    def __init__(self, screen_temp, x, y, plane):
        BaseBullet.__init__(self, screen_temp, x, y,
                            "./img/" + bullet_type[plane.plane_type], plane)
...
class EnemyBullet(BaseBullet):
    ...
    def __init__(self, screen_temp, x, y, plane):
        BaseBullet.__init__(self, screen_temp,
                            x + plane_size[plane.plane_type]["width"] / 2,
                            y + plane_size[plane.plane_type]["height"] / 2,
                            "./img/" + bullet_type[plane.plane_type],
                            plane)  # x, y 为子弹发射的位置
...
class supply_2_hero(BaseBullet):
    ...
    def __init__(self, screen_temp, x, y, suppl_type_temp, speed_temp, s_HP):
        BaseBullet.__init__(self, screen_temp, x, y,
                            "./img/" + supply_image[suppl_type_temp], None)
...
def create_enemy_plane():
    ...
    # enemy0
    if (random_num == 20
            or random == 40) and len(enemy0_list) < enemy0_maximum:
        enemy0_list.append(Enemy0Plane(window_screen))
    # enemy1
    if (hit_score >= random_appear_boss1 and (hit_score % random_appear_boss1)
            == 0) and len(enemy1_list) < enemy1_maximum:
        enemy1_list.append(Enemy1Plane(window_screen))
    # enemy2
    if (hit_score >= random_appear_boss2 and (hit_score % random_appear_boss2)
            == 0) and len(enemy2_list) < enemy2_maximum:
...
def create_supply_2_hero(s_type):
    ...
    if (random_supply % 690) == 0 and s_type == 0:  # 血量补给
        ...
        bullet_supply = supply_2_hero(window_screen,
                                      ...
                                      random.randint(-115, -108), s_type, 3, 0)
...
def enemy_display_move_fire(enemy):
    ...
    if hero:  # 飞机击中判断
        hero.isHitted(enemy, plane_size[hero.plane_type]["width"],
...
def show_score_HP():
    ...
    if hero:  # hero对象存在
        ...
        window_screen.blit(number_image[three_bullet_stock_temp[0]],
                           (605, 600))
        window_screen.blit(number_image[three_bullet_stock_temp[1]],
                           (635, 600))
        window_screen.blit(number_image[three_bullet_stock_temp[2]],
                           (665, 600))
    else:  # hero对象不存在
        # 贴score_hp，单管，三管子弹余量
        window_screen.blit(score_hp_image, (480, 460))
...
def image_load():
    ...
    try:
        # 暂停图片
        pause_image = pygame.image.load("./img/btn_finish.png")
        # 右侧图片导入
        background = pygame.image.load("./img/background.png")
        restart = pygame.image.load("./img/restart_nor.png")
        exit_game = pygame.image.load("./img/quit_nor.png")
        description = pygame.image.load("./img/description.png")
        # 数字图片导入
        for i in range(10):
            number_image.append(
                pygame.image.load("./img/number_" + str(i) + ".png"))
        # score_hp导入
        score_hp_image = pygame.image.load("./img/score_hp.png")
        # 单三管炮弹照片
        one_or_three_barral.append(
            pygame.image.load("./img/bullet_temp1.png"))  # 单管
        one_or_three_barral.append(
            pygame.image.load("./img/bullet_temp3.png"))  # 三管
        # 三管炮弹子弹余量照片
        bullet_3_stock = pygame.image.load("./img/bullet_3_stock.png")
        # 最高分
        max_score_image = pygame.image.load("./img/max_score.png")
        # boss_hp
        boss_HP_image = pygame.image.load("./img/boss_HP.png")
        # line_image
...
def background_music_load():
    try:
        ...
        pygame.mixer.music.load(
            "./music/PlaneWarsBackgroundMusic.ogg")  # 游戏背景音乐
        pygame.mixer.music.set_volume(0.3)  # 设置音量(0-1)
        pygame.mixer.music.play(-1)  # 循环播放
        hero_fire_music = pygame.mixer.Sound(
...
def main():
    ...
    while True:
        ...
        if hero and blood_supply:
            hero.supply_hitted(blood_supply,
                               plane_size[hero.plane_type]["width"],
                               plane_size[hero.plane_type]["height"])
        if hero and bullet_supply:
            hero.supply_hitted(bullet_supply,
                               plane_size[hero.plane_type]["width"],
                               plane_size[hero.plane_type]["height"])
        # enemy0
        if enemy0_list:
